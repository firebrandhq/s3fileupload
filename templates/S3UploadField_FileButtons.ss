<% if $canView %>
	<a class="ss-ui-button ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary" target="_blank" data-icon="download-csv" href="$TemporaryDownloadLink" role="button" aria-disabled="false">
		Download
		<span class="ui-button-icon-primary ui-icon btn-icon-download-csv"></span>
	</a>
<% end_if %>
<% if $canEdit %>
